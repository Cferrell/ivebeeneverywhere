<div class="row-fluid pricing-table pricing-three-column" style="margin-top: 10px; display:block; width:100%; overflow:hidden; background:white; box-shadow: 0 0 5px hsla(0, 0%, 20%, 0.3);padding-bottom:70px">
	<div class="plan-name" style="margin-top:20px;text-align: center;">
        <h2 style="font-weight: bold;font-size: 36px;padding-top: 30px;padding-bottom: 10px;color:#D9534F;">Ultimate Responsive Image Slider Pro</h2>
		<h6 style="font-size: 21px;padding-top: 10px;padding-bottom: 10px;margin-left:11px;font-family: Open Sans,sans-serif;
    line-height: 1.6em;">Ultimate Responsive Image Slider Pro comes with 5 different slider layout.</h6>
    </div>
	<hr>
	<div class="purchase_btn_div" style="margin-top:20px; margin-left:30px;">
		...
		<a  style= "margin-right:10px; margin-left:40px; margin-top:30px; text-decoration:none;" href="https://weblizar.com/forum/" target="_new" class="btn btn-primary btn-lg">View Support Docs or Open a Ticket</a>		
	</div>
	<hr>
...
if(isset($_REQUEST['urispchangeurl']) ) {		
	$all_posts = wp_count_posts( 'risp_gallery')->publish;
	$args = array('post_type' => 'risp_gallery', 'posts_per_page' =>$all_posts);
	global $URISP_Sliders;
	$URISP_Sliders = new WP_Query( $args );			

	while ( $URISP_Sliders->have_posts() ) : $URISP_Sliders->the_post();	
		$URISP_Id = get_the_ID();
		$RISP_AllPhotosDetails = unserialize(base64_decode(get_post_meta( $URISP_Id, 'risp_all_photos_details', true)));
		$TotalImages =  get_post_meta( $URISP_Id, 'risp_total_images_count', true );
		if($TotalImages) {
			...
			update_post_meta($URISP_Id, 'risp_all_photos_details', base64_encode(serialize($ImagesArray)));
			$ImagesArray="";
		}	
	endwhile; 
}
